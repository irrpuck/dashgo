...
class Arduino:
    ...
    def drive_m_per_s(self, right, left):
        ...
        left_ticks_per_loop = int(
            left_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        right_ticks_per_loop = int(
            right_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
...
class BaseController:
    ...
    def poll(self):
        ...
        if abs(self.v_des_left - self.v_left) > 0:
            vold = self.v_left
            # accelerate
            self.v_left += self.max_accel
            rospy.loginfo("accelerating old: %f, new: %f", vold, self.v_left)
            # hit max
            if abs(self.v_left > self.v_des_left):
                self.v_left = self.v_des_left

        elif abs(self.v_des_left - self.v_left) < 0:
            vold = self.v_left
            # decelerate
        ...
        if abs(self.v_des_right - self.v_right) > 0:
            # accelerate
            self.v_right += self.max_accel
            # hit max
            if abs(self.v_right > self.v_des_right):
                self.v_right = self.v_des_right

        elif abs(self.v_des_right - self.v_right) < 0:
